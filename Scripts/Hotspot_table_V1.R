...
template= read.table(files[1], sep = "\t", header=TRUE) # read template .txt file
...
data = DSBList
BG = read.table("BGreads.txt", sep = "\t", header=TRUE) #Import background datatable
...
r=foreach (k = strains) %dopar%
  ...
  parent.directory
  out = paste("T_0.193_Hotspot.Table.",dflistNames[k],".txt", sep="") # Set hotspot table name
